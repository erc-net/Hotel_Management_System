...
class Cust_Win:  
  ...
  def __init__(self,root):
    ...
    #age
    lblmname=Label(LabelFrameleft,text="Age:",font=("arial",12,"bold"),padx=2,pady=6)
    lblmname.grid(row=2,column=0,sticky=W)
    
    txtmname=ttk.Entry(LabelFrameleft,textvariable=self.var_age,width=29,font=("arial",13,"bold"))
    ...
    txtNationality=ttk.Entry(LabelFrameleft,textvariable=self.var_nationality,width=29,font=("arial",13,"bold"))
    ...
    combo_Id["value"]=("Driving Licence","Passport","National Id Card",)
    combo_Id.current(0)
    ...
    btnSearch=Button(Table_frame,command=self.search,text="Search",font=("arial",13,"bold"),bg="black",fg="white",width=9)
    btnSearch.grid(row=0,column=3,padx=1)
    
    btnShowAll=Button(Table_frame,command=self.fetch_data,text="Show All",font=("arial",13,"bold"),bg="black",fg="white",width=9)
    ...
    self.Cust_Details_Table=ttk.Treeview(details_table,column=("ref","name","age","gender","post","mobile","email","nationality","idproof","idnumber","address"),xscrollcommand=scroll_x.set,yscrollcommand=scroll_y.set)
  ...
  def update(self):
    if self.var_mobile.get()=="":
      ...
      conn=mysql.connector.connect(host="localhost",username="root",password="eric27",database="management")
      my_cursor=conn.cursor()
      my_cursor.execute("UPDATE customer SET Name=%s, Age=%s, Gender=%s, PostCode=%s, Mobile=%s, Email=%s, Nationality=%s, Idproof=%s, idnumber=%s, Address=%s WHERE Ref=%s",(
  ...
  def search(self):
   ...
   # my_cursor.execute("select * from customer where"+str(self.serch_var.get())+" LIKE '%"+str(self.txt_search.get())+"%'")
    ...
    sql_query = "SELECT * FROM customer WHERE " + str(self.serch_var.get()) + " LIKE %s"
